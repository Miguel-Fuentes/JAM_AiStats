...
class AIM(Mechanism):
    ...
    def worst_approximated(self, candidates, answers, model, eps, sigma):
        ...
        return self.exponential_mechanism(errors, eps, 2 * max_sensitivity)
    ...
    def run(self, data, W):
        ...
        sigma = np.sqrt(rounds * 2 / (2*0.9*self.rho)) ## sensitivity^2
        epsilon = np.sqrt(8*0.1*self.rho/rounds)
       
        measurements = []
        print('Initial Sigma', sigma)
        rho_used = len(oneway)/sigma**2 ## removed 0.5 in numerator
        for cl in oneway:
        ...
        while not terminate:
            ...
            if self.rho - rho_used < 2*(1/sigma**2 + 1.0/8 * epsilon**2): ## removed 0.5 in numerator
                ...
                sigma = np.sqrt(2 / (2*0.9*remaining)) ## added 2 in numerator
                epsilon = np.sqrt(8*0.1*remaining)
                terminate = True
                
            rho_used += 1.0/8 * epsilon**2 + 1/sigma**2 ## removed 0.5 in numerator
